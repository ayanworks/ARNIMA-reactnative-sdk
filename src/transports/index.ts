...
class InboundMessageHandler {
  ...
  inboundMessageStatusListener = EventRegister.addEventListener(
    ...
    async () => {
      ...
      const unprocessedMessages: Array<Record> =
        await WalletStorageService.getWalletRecordsFromQuery(
          JSON.parse(this.wallet.walletConfig),
          JSON.parse(this.wallet.walletCredentials),
          RecordType.SSIMessage,
  ...
  proceedInboundMessage = async () => {
    try {
      const query: Object = {isProcessed: JSON.stringify(false)};
      let unprocessedMessages: Array<Record> =
        await WalletStorageService.getWalletRecordsFromQuery(
          JSON.parse(this.wallet.walletConfig),
          JSON.parse(this.wallet.walletCredentials),
          RecordType.SSIMessage,
      ...
      for (let i = 0; i < unprocessedMessages.length; i++) {
        ...
        if (unprocessedMessages[i].tags.autoProcessed === 'true') {
          ...
          const unpackMessageResponse = await unpackMessage(
            JSON.parse(this.wallet.walletConfig),
            JSON.parse(this.wallet.walletCredentials),
            messageRecord,
          );
          ...
          switch (message['@type']) {
            ...
            case MessageType.TrustPingResponseMessage: {
              const connection: Connection =
                await TrustPingService.saveTrustPingResponse(
                  JSON.parse(this.wallet.walletConfig),
                  JSON.parse(this.wallet.walletCredentials),
                  unpackMessageResponse,
                );
            ...
            case MessageType.MediationGrant: {
              const isCompleted = await MediatorService.saveRoutingKeys(
                message,
              );
              if (isCompleted) {
                await WalletStorageService.deleteWalletRecord(
