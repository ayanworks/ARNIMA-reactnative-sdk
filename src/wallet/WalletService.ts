...
class WalletService {
  async createWallet(
    ...
    try {
      ...
      return await this.createWalletDidStore(
        config,
        credentials,
        {},
        true,
  ...
  async deleteWallet(): Promise<boolean> {
    try {
      const sdkDB: any = DatabaseServices.getWallet();
      await ArnimaSdk.closeWallet();
      const response: boolean = await ArnimaSdk.deleteWallet(
        sdkDB.walletConfig,
        sdkDB.walletCredentials,
      );
      await DatabaseServices.resetWallet();
      return response;
    } catch (error) {
      console.log('WalletService - Delete wallet error = ', error);
      throw error;
    }
  }
}
